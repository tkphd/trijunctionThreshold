...
void generate(int dim, char* filename)
{
	...
	if (dim == 2)	{
		...
		// Create seeds for tessellation
		std::vector<MMSP::vector<int> > local_seeds;
		std::vector<std::vector<MMSP::vector<int> > > seeds;
		while (int(seeds.size()) <= rank)
			seeds.push_back(local_seeds);

		MMSP::vector<int> s(3,0);
		// Central seed (grain zero, the triangle)
		s[0] = Lx/2;
		s[1] = (4.0 + 2.0/3) / 8.125 * Ly;
		...
		seeds[rank] = local_seeds;

		tessellate(local_seeds, seeds, initGrid);
