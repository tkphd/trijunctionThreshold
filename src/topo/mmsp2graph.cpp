...
template <typename T> void sparse_graph(const MMSP::grid<2,MMSP::sparse<T> >& grid, std::string altfile);


template <typename T> void sparse_graph(const MMSP::grid<3,MMSP::sparse<T> >& grid, std::string altfile);


int main(int argc, char* argv[])
{
	if ( argc != 3 ) {
		std::cout << "Usage: " << argv[0] << " input.dat output.csv\n";
		return ( 1 );
	}
...
template <typename T> void sparse_graph(const MMSP::grid<2,MMSP::sparse<T> >& grid, std::string altfile)
{
	...
	invariant=0;
	double unitsz=1.0;
	for (int d=0; d<2; d++) unitsz*=MMSP::dx(grid,d);
